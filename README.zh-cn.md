# cap4j 　　　　　　　　　　　　　　　　　　　　[English](https://github.com/netcorepal/cap4j/blob/main/README.md)
...
## 快速开始
cap4j要解决的是如何实现领域驱动设计的问题，想要用好cap4j，最好是基于对[整洁架构](https://blog.cleancoder.com/uncle-bob/2012/08/13/the-clean-architecture.html)、领域模型以及CQS模式有初步的认识。

当然如果你目前对领域模型和CQS模式不甚了解也不要紧。通过使用该项目来构建你的服务，你会对这些概念有初步的了解。反向可以帮助你对领域驱动设计加深理解。
...
@PostMapping("/$cmd$")
public ResponseData<$ReturnType$> $cmd$(@RequestBody @Valid $Cmd$ cmd) {
    $ReturnType$ result = $cmd$Handler.exec(cmd);
...
@GetMapping("/$qry$")
public ResponseData<$Qry$.$Qry$Dto> $qry$($Qry$ param) {
...
@GetMapping("/$qry$")
public ResponseData<List<$Qry$.$Qry$Dto>> $qry$($Qry$ param) {
...
@PostMapping("/$qry$")
public ResponseData<PageData<$Qry$.$Qry$Dto>> $qry$(@RequestBody $Qry$ param) {
